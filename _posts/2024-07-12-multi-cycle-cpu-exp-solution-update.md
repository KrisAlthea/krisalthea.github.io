---
title: "multi-cycle-cpu-exp-solution-update"
date: 2024-07-12
---
...
接下来有个方向, 一个是添加文件, 另一个是IP实例化,这里选择第二种
...
1. name: `inst_rom` 
   ...
   2. Port A 选项卡
      ...
      4. Primitives Output Register 取消勾选
...
好像30H：`sw $6,0($5)`并不能存进去，60H：`lw $11,0($8)`读的时候也读不出来。

最后还是使用了$0，4CH、50H：`sw $6,#28($0)`和`lw $12,#28($0)`，能正常存入和读出。

14H、20H、30H、54H到78H这些代码就变得没有意义了（除了凑数）

```assembly
--指令地址------汇编指令-----------------结果描述----
00H             addiu $1,$0,#10         [$1] = 0000_000AH    ; 初始化N = 10
04H             ori $2,$0,#0            [$2] = 0000_0000H    ; 用于存储计数
08H             ori $3,$0,#1            [$3] = 0000_0001H    ; 第一个斐波那契数
0CH             addu $4,$3,$0           [$4] = 0000_0001H    ; 第二个斐波那契数
10H             andi $5,$0,#0           [$5] = 0000_0000H    ; 地址偏移量初始化
14H             sw $3,0($5)             Mem[0000_0000H] = 0000_0001H  ; 存储第一个数
18H             addiu $5,$5,#4          [$5] += 0000_0004H   ; 更新地址
1CH             addiu $2,$2,#1          [$2] += 0000_0001H   ; 更新计数
20H             sw $4,0($5)             Mem[0000_0004H] = 0000_0001H  ; 存储第二个数
24H             addiu $5,$5,#4          [$5] += 0000_0004H   ; 更新地址
28H             addiu $2,$2,#1          [$2] += 0000_0001H   ; 更新计数
2CH             addu $6,$3,$4           [$6] = [$3] + [$4]  ; 计算下一个斐波那契数
30H             sw $6,0($5)             Mem[$5] = $6        ; 存储计算结果
34H             addiu $5,$5,#4          [$5] += 0000_0004H   ; 更新地址
38H             addiu $2,$2,#1          [$2] += 0000_0001H   ; 更新计数
3CH             addu $3,$4,$6           [$3] = $4 + $6      ; 更新前两个数
40H             addu $4,$6,$3           [$4] = $6 + $3      ; 更新前两个数
44H             sltu $7,$2,$1           [$7] = ($2 < $1)    ; 检查是否计数达到N
48H             bne $7,$0,#-13           if $7 != 0 跳回14H ; 继续计算下一项
4CH             sw $6,#28($0)           Mem[1C]=$6         ; 1CH存$6
50H             lw $12,#28($0)          $12 = Mem[1C]      ; 取值到$12
54H             xor $8,$0,$0            [$8] = 0000_0000H  ; 清零寄存器$8
58H             lui $9,0x0001           [$9] = 0001_0000H  ; 初始化步进值
5CH             addiu $10,$0,#1         [$10] = 1          ; 输出计数器
60H             lw $11,0($8)            [$11] = Mem[$8]    ; 从内存载入数值
64H             addiu $8,$8,#4          [$8] += 0000_0004H ; 更新内存地址
68H             addiu $10,$10,#-1       [$10] -= 0000_0001H ; 减少输出计数器
6CH             bne $10,$0,#-3          if $10 != 0 跳回58H ; 继续读取下一个数
70H             addiu $3,$3,#1          [$3] += 0000_0001H ; 增加$3
74H             sltu $4,$3,$5           [$4] = ($3 < $5)    ; 检查$3是否小于5
78H             bne $4,$0,#-2           if $4 != 0 跳回68H ; 继续增加
7CH             j 00H                   跳转到00H地址        ; 程序循环执行
```
...
**sw** rt,offset(**b**)  -----101011|**b**|**rt**|**offset**
...
**lw** rt,offset(**b**)  -----100011|**b**|**rt**|**offset**

**j** target     ------000010|**target**

以下是指令的二进制格式及其对应的十六进制格式的表格：

| 汇编指令          | 二进制格式                             | 十六进制 |
| ----------------- | -------------------------------------- | -------- |
| addiu $1,$0,#10   | 001001 00000 00001 0000 0000 0000 0010 | 2401000A |
| ori $2,$0,#0      | 001101 00000 00010 0000 0000 0000 0000 | 34020000 |
| ori $3,$0,#1      | 001101 00000 00011 0000 0000 0000 0001 | 34030001 |
| addu $4,$3,$0     | 000000 00011 00000 00100 00000 100001  | 00602021 |
| andi $5,$0,#0     | 001100 00000 00101 0000 0000 0000 0000 | 30050000 |
| sw $3,0($5)       | 101011 00101 00011 0000 0000 0000 0000 | AC230000 |
| addiu $5,$5,#4    | 001001 00101 00101 0000 0000 0000 0100 | 24A50004 |
| addiu $2,$2,#1    | 001001 00010 00010 0000 0000 0000 0001 | 24420001 |
| sw $4,0($5)       | 101011 00101 00100 0000 0000 0000 0000 | AC240000 |
| addiu $5,$5,#4    | 001001 00101 00101 0000 0000 0000 0100 | 24A50004 |
| addiu $2,$2,#1    | 001001 00010 00010 0000 0000 0000 0001 | 24420001 |
| addu $6,$3,$4     | 000000 00011 00100 00110 00000 100001  | 00643021 |
| sw $6,0($5)       | 101011 00101 00110 0000 0000 0000 0000 | AC260000 |
| addiu $5,$5,#4    | 001001 00101 00101 0000 0000 0000 0100 | 24A50004 |
| addiu $2,$2,#1    | 001001 00010 00010 0000 0000 0000 0001 | 24420001 |
| addu $3,$4,$6     | 000000 00100 00110 00011 00000 100001  | 00861821 |
| addu $4,$6,$3     | 000000 00110 00011 00100 00000 100001  | 00C32021 |
| sltu $7,$2,$1     | 000000 00010 00001 00111 00000 101011  | 0041382B |
| bne $7,$0,#-13    | 000101 00111 00000 1111 1111 1111 0011 | 14E0FFF3 |
| sw $6,#28($0)     | 101011 00000 00110 0000 0000 0001 1100 | AC06001C |
| lw $12,#28($0)    | 100011 00000 01100 0000 0000 0001 1100 | 8C0C001C |
| xor $8,$0,$0      | 000000 00000 00000 01000 00000 100110  | 00004026 |
| lui $9,0x0001     | 001111 00000 01001 0000 0000 0000 0001 | 3C090001 |
| addiu $10,$0,#1   | 001001 00000 01010 0000 0000 0000 0001 | 240A0001 |
| lw $11,0($8)      | 100011 01000 01011 0000 0000 0000 0000 | 8D0B0000 |
| addiu $8,$8,#4    | 001001 01000 01000 0000 0000 0000 0100 | 25080004 |
| addiu $10,$10,#-1 | 001001 01010 01010 1111 1111 1111 1111 | 254AFFFF |
| bne $10,$0,#-3    | 000101 01010 00000 1111 1111 1111 1101 | 1540FFFD |
| addiu $3,$3,#1    | 001001 00011 00011 0000 0000 0000 0001 | 24630001 |
| sltu $4,$3,$5     | 000000 00011 00101 00100 00000 101011  | 0065202B |
| bne $4,$0,#-2     | 000101 00100 00000 1111 1111 1111 1110 | 1480FFFE |
| j 00H             | 000010 0000 0000 0000 0000 0000 0000   | 08000000 |
...
48H             bne $7,$0,#-13           if $7 != 0 跳回14H ; 继续计算下一项
4CH             sw $6,#28($0)           Mem[1C]=$6         ; 1CH存$6
50H             xor $8,$0,$0            [$8] = 0000_0000H  ; 清零寄存器$8
54H             lw $2,0($8)             [$2] = Mem[$8]    ; 从内存载入数值
58H             lw $3,4($8)             [$3] = Mem[$8+4]    ; 从内存载入数值
5CH             lw $4,8($8)             [$4] = Mem[$8+8]    ; 从内存载入数值
60H             lw $5,C($8)             [$5] = Mem[$8+C]    ; 从内存载入数值
64H             lw $6,10($8)            [$6] = Mem[$8+10]    ; 从内存载入数值
68H             lw $7,14($8)            [$7] = Mem[$8+14]    ; 从内存载入数值
6CH             lw $8,18($8)            [$8] = Mem[$8+18]    ; 从内存载入数值
70H             lw $9,1C($8)            [$9] = Mem[$8+1C]    ; 从内存载入数值
74H             lw $10,20($8)           [$10] = Mem[$8+20]    ; 从内存载入数值
78H             lw $11,24($8)           [$11] = Mem[$8+24]    ; 从内存载入数值
7CH             j 00H                   跳转到00H地址        ; 程序循环执行
```
...
| bne $7,$0,#-13  | 000101 00111 00000 1111 1111 1111 0011 | 14E0FFF3 |
| xor $8,$0,$0    | 000000 00000 00000 01000 00000 100110  | 00004026 |
| lw $2,0($8)     | 100011 01000 00010 0000 0000 0000 0000 | 8D020000 |
| lw $3,4($8)     | 100011 01000 00011 0000 0000 0000 0100 | 8D030004 |
| lw $4,8($8)     | 100011 01000 00100 0000 0000 0000 1000 | 8D040008 |
| lw $5,C($8)     | 100011 01000 00101 0000 0000 0000 1100 | 8D05000C |
| lw $6,10($8)    | 100011 01000 00110 0000 0000 0001 0000 | 8D060010 |
| lw $7,14($8)    | 100011 01000 00111 0000 0000 0001 0100 | 8D070014 |
| lw $8,18($8)    | 100011 01000 01000 0000 0000 0001 1000 | 8D080018 |
| lw $9,1C($8)    | 100011 01000 01001 0000 0000 0001 1100 | 8D09001C |
| lw $10,20($8)   | 100011 01000 01010 0000 0000 0010 0000 | 8D0A0020 |
| lw $11,24($8)   | 100011 01000 01011 0000 0000 0010 0100 | 8D0B0024 |
| j 00H           | 000010 0000 0000 0000 0000 0000 0000   | 08000000 |
